...
/*
  ...
  Sensors:
   ...
   - IR Proximity Sensor (Waste insertion trigger)
   - MQ135 Gas Sensor (Air quality analog)
   - DHT11 (Temperature/Humidity)
  Network:
   - WiFi + HTTP POST JSON

  Loop timing: every 5 seconds

  =========================
  WIRING / PIN CONFIG NOTES
  =========================

  ESP32 uses 3.3V logic. Be careful with 5V sensors:
  - HC-SR04 ECHO is often 5V -> use a voltage divider or level shifter to ESP32 input.
  - HX711 recommended at 3.3V with ESP32.
  - MQ135 module may output up to 5V -> ensure ADC pin sees <= 3.3V (divider), or power module at 3.3V if supported.

  Example wiring (adjust pins below as needed):
  ...
  HC-SR04:
    ...
    ECHO -> GPIO 18  (ECHO_PIN)  [via divider/level shift]

  HX711:
    VCC -> 3.3V
    GND -> GND
    DT  -> GPIO 19   (HX711_DT)
    SCK -> GPIO 23   (HX711_SCK)

  MQ135 (analog):
    VCC -> 5V/3.3V (module dependent)
    GND -> GND
    AO  -> GPIO 34 (MQ135_PIN)   [ensure <=3.3V]
